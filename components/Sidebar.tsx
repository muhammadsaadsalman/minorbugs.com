...
import { useState } from "react";
import Select from "react-select";
import { NavItems, NavItemProps, SubMenuItem } from "@/public/data/home.data";
...
const Sidebar = ({ sidebar, setSidebar }: sideBarProps) => {
  ...
  const renderNavItem = (
      item: NavItemProps | SubMenuItem,
      index: number,
      parentIndex?: number
  ): JSX.Element => {
    const hasSubItems = item.subItems && item.subItems.length > 0;
    const dropdownIndex = parentIndex !== undefined ? parentIndex : index;

    if (hasSubItems) {
      // Has sub-items - render as dropdown
      return (
          <li key={index} className="dropdown show-dropdown">
            <button
                ...
                className="dropdown-toggle dropdown-nav fs-three heading"
                onClick={() =>
                    setDropdown((prev) => (prev === dropdownIndex ? null : dropdownIndex))
                }
            >
              {item.label}
            </button>
            <ul className={`dropdown-menu ${dropdown === dropdownIndex && "d-block"}`}>
                  ...
                  return (
                      <li key={subIndex} className="sub-dropdown">
                        <button
                            ...
                            className="dropdown-item fs-three heading dropdown-toggle"
                            onClick={() =>
                                setSubmenu((prev) => (prev === subIndex ? null : subIndex))
                            }
                        >
                          {subItem.label}
                        </button>
    ...
    // No sub-items - render as simple link
    return (
        <li key={index}>
          <Link
              className="dropdown-nav fs-three heading"
  ...
  return (
      // <div className={`sidebar-wrapper ${sidebar ? "sidebar-active" : ""}`}>
      <div className={`sidebar-wrapper ${sidebar ? "active-mobile" : ""}`}>

      <div className="position-relative">
          <div className="side-menubar py-6 d-flex flex-column justify-content-between align-items-center">
            {/*<div className="sidebar-close d-none d-xl-block">*/}
            <div className="sidebar-close d-block d-xl-block">

            <button
                  onClick={() => setSidebar(!sidebar)}
                  className="d-center d-grid"
                  type="button"
